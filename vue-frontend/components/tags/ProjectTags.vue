<template>
  ...
  <div class="c-project-tags">
    ...
    <el-info-box style="margin-top: 20px;" :image="require('@/assets/info_picture.svg')">
      ...
      <el-button type="secondary" style="margin-top: 20px;" icon="add"
                 @click="addProjectTag">
        {{ $t('workspace.addTag') }}
      </el-button>
    </el-info-box>
    ...
    <el-table style="display: flex" v-if="$store.state.tagsState.tags && $store.state.tagsState.tags.length" :columns="tagsColumns" :fixed-header="true"
...
export default {
  ...
  methods: {
    addProjectTag() {
      if (this.$store.getters["workspacesState/getActiveWorkspaceUserLicenseValid"]) this.$store.commit('tagsState/mutateEditedTag', {relation: 'Project'})
  ...
  beforeMount() {
    this.$store.dispatch('tagsState/actionGetTagSetsProject')
  },
  beforeDestroy() {
